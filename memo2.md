...
stealとchoice機能に関して、作成したいです。
まずはfrontendのコードから修正します。
その後Backendのファイル（EffectService.ts）を確認して実装を追加、または変更点がないか確認して修正してください。

# プレイヤーにLoading中であることを示す機能

-   GameBoardに選択中...や、待機中...等をアナウンス出来るコンポーネントを追加してください。
    -   プレイヤーが例えば他のプレイヤーの選択を待機している状態で、その待ち状態を把握させるために使用したいです。
        -   プレイヤーはこのコンポーネントは別のコンポーネントから、選択した内容をサーバーにemitします。
        -   全てのプレイヤーが選択をし終わったことをbackendで判定します。

# プレイヤーを選択するボタンコンポーネント

-   backendの添付ファイルにあるEffectService.tsで効果処理の計算を行います。
    -   もしユーザ指定を行う必要がある効果処理（ここではsteal）を行う必要があった場合、そのプレイヤーに対してemitして
        ユーザを指定させるためのボタンを表示させます。
-   OtherPlayer.tsxコンポーネントのそれぞれのプレイヤーのCage領域の左側に、このプレイヤーから{{奪う}}、という名称のボタンを表示させます。奪うは変数で、いろんな文言にも対応できるようにしてほしい。
-   ユーザが、自分以外のいずれかのプレイヤーを選択したとき、そのプレイヤー情報を付与してbackendにemitします。
-   backendは選択情報を受け取り続きの（ここではSteal等）の処理を開始します。

# 選択肢を選ばせるボタンコンポーネント

-   backendの添付ファイルにあるEffectService.tsで効果処理の計算を行います。
    -   もしユーザが選択を行う必要がある効果処理（ここではchoise）を行う必要があった場合、そのプレイヤーに対してemitして
        必要な選択ボタンを表示させます。
        -   選択モーダル画面のデザインは、ResultDisplayと同様のデザインにして、その画面上に選択肢のボタンを表示させてください。
    -   ユーザが効果を選択したら、backendにその選択肢をemitします。
    -   選択された効果処理を行います。

# 効果処理までの流れ

    - useSocketIO.tsxに下記の処理を追加します。
    - 処理が終わった事を判定したら、backendから、待機中のアニメーションを停止するためのemitをfrontend側に行います。
    - プレイヤーがbackendからemitしたことを受け取ったら、ActionPhaseNotifier.tsxで、効果処理開始の文字を表示します。
    - 効果処理開始のアニメーションが終わったことをbackendに通知します。
        - LockStepManager.tsでタイミングを合わせて、効果処理アニメーションを開始します。

# 効果処理結果の表現

効果処理結果をユーザごとに配列に設定し、ユーザごとに表示するようにしたいです。

-   新しくEffectResultsというtypeを用意してその配列型に結果を格納します。
-   全プレイヤーの効果処理が全て終わってから下記を表示してほしいです。

# 効果処理結果のフォーマット

-   ResultDisplay.tsxのデザインと同様にして順番に下記を表示させる

    {{動物のアイコン}} ：効果の内容（例：コイン＋x、コインを～から奪う等）
    : {{動物が複数いたらここに複数行表示}}

合計 : コイン : {{処理後のコインの所持数}} ({コインの増減（+何枚または-何枚）})
スター : {{処理後のスター数}} ({{スターの増減（+-何枚かを表示）}})

-   全員分の表示が終わったら、それぞれのfrontendのクライアントからuseSocketIOでemitして、効果処理アニメーションが終わったことをbackendに通知します。

backendでは、GameService、GameController、EffectService、SocketEvents、useSocketIO
frontendでは、OtherPlayer、OtherPlayersSection
上記のファイル等で、修正するファイルがあるかどうか確認してください。

またこれまでの処理の流れや、実装に関して必要なことがあれば質問してください。
